...
import {
  ...
  Camera, BookOpen, List, FileEdit, Info, RotateCcw, AlignLeft, FileImage
} from 'lucide-react';
...
const categories = [
  {
    ...
    tools: [
      ...
      { title: 'Split PDF', description: 'Extract pages or split into separate files.', icon: Scissors, href: '/tools/split-pdf' },
      { title: 'Reorder Pages', description: 'Drag & drop to rearrange PDF pages.', icon: RotateCcw, href: '/tools/reorder-pages' },
      { title: 'Delete Pages', description: 'Remove unwanted pages from your PDF.', icon: Trash2, href: '/tools/delete-pdf-pages' },
  ...
  },
  {
    name: 'Edit',
    tools: [
      { title: 'Add Text', description: 'Place custom text anywhere on PDF pages.', icon: MousePointer2, href: '/tools/add-text' },
      ...
      { title: 'Add Signature', description: 'Sign your PDFs with drawn or typed signatures.', icon: PenTool, href: '/tools/add-signature' },
      ...
      { title: 'Edit Metadata', description: 'Change title, author, and PDF properties.', icon: FileEdit, href: '/tools/edit-metadata' },
  ...
  },
  {
    ...
    tools: [
      ...
      { title: 'Compress PDF', description: 'Reduce PDF file size, keep quality.', icon: FileDown, href: '/tools/compress-pdf' },
    ]
  },
  {
  ...
  },
  {
    ...
    tools: [
      ...
      { title: 'Text to PDF', description: 'Convert plain text into a PDF document.', icon: AlignLeft, href: '/tools/text-to-pdf' },
    ]
  },
  {
    name: 'Create',
        ...
        {categories.map((cat) => (
          <div key={cat.name}>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-6 gap-3">
              {cat.tools.map((tool) => (
                <ToolCard key={tool.title} {...tool} category={cat.name} />
              ))}
            </div>
          </div>
        ))}
